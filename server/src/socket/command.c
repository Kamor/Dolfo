...
CommArray_s CommandsGM[] =
{
    ...
    {"setserverexp",   command_setserverexp,   0.0f, 1, NULL},
    {"setplayerexp",   command_setplayerexp,   0.0f, 1, NULL},
...
void cs_cmd_moveobj(char *buf, int len, NewSocket *ns)
{
    ...
    if (!loc &&
        (!pl->container ||
         !pl->container->map))
    {
        what = thing_drop_to_floor(who, what, nrof);
    }
    /* Pick up. */
    else
    {
        ...
        if (where == who ||
            (what == pl->container &&
             where == what))
        {
            what = thing_pick_up(who, what, NULL, nrof);
        }
        /* Pick up to container. */
        else
        {
            what = thing_pick_up(who, what, where, nrof);
        }
    }
}
    ...
    switch (mode)
    {
        case ALTACT_MODE_ARCHERY:
            if ((weapon = pl->equipment[PLAYER_EQUIP_BOW]) &&
                ...
                weapon->type == BOW &&
                ammo->type == ARROW &&
                ((weapon->subtype == RANGE_WEAP_BOW &&
                  ammo->subtype == SUBTYPE_MISSILE_BOW &&
                  change_skill(op, SK_RANGE_BOW)) ||
                 (weapon->subtype == RANGE_WEAP_XBOWS &&
                  ammo->subtype == SUBTYPE_MISSILE_CBOW &&
                  change_skill(op, SK_RANGE_XBOW)) ||
                 (weapon->subtype == RANGE_WEAP_SLINGS &&
                  ammo->subtype == SUBTYPE_MISSILE_SSTONE &&
        ...
        case ALTACT_MODE_THROWING:
            if ((weapon = pl->equipment[PLAYER_EQUIP_BOW]) &&
                weapon->type == ARROW &&
                (weapon->subtype & SUBTYPE_MISSILE_THROW) &&
