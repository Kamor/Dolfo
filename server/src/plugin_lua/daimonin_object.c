...
static struct method_decl GameObject_methods[] =
{
    ...
    {"IdentifyItem",           (lua_CFunction) GameObject_IdentifyItem}, // deprecated
    ...
    {"MakeEgo",                (lua_CFunction) GameObject_MakeEgo},
...
struct attribute_decl GameObject_attributes[] =
{
    ...
    {"egomode",               FIELDTYPE_SINT8,     offsetof(object_t, egomode),                   FIELDFLAG_READONLY, 0},
    {"egoname",               FIELDTYPE_SHSTR,     offsetof(object_t, egoname),                   FIELDFLAG_READONLY, 0},
    ...
    {"sub_type_1",            FIELDTYPE_UINT8,     offsetof(object_t, subtype),                   0,                  0}, // deprecated, use subtype
    {"subtype",               FIELDTYPE_UINT8,     offsetof(object_t, subtype),                   0,                  0},
...
static int GameObject_setAttribute(lua_State *L, lua_object *obj, struct attribute_decl *attrib, int before)
{
    ...
    {
        /* TODO: This area of code (the active list) is already on my near
	 * future todo list but this'll do for right now.
	 *
	 * -- Smacky 20201125 */
        if (attrib->offset == offsetof(object_t, speed))
        {
            hooks->update_ob_speed(who);
        }

        /* recalculate carrying when a script changes an inventory object_t's
         * weight or direct nrof. */
        if (who->env &&
            !QUERY_FLAG(who, FLAG_SYS_OBJECT) &&
            (attrib->offset == offsetof(object_t, weight) ||
             attrib->offset == offsetof(object_t, nrof)))
        {
            object_t *e;

            for (e = who->env; e->env; e = e->env)
            {
                if (QUERY_FLAG(e, FLAG_SYS_OBJECT))
                {
                    break;
                }

                e->carrying = hooks->sum_weight(e);
            }
        }
...
static int GameObject_setFlag(lua_State *L, lua_object *obj, uint32 flagno, int before)
{
    ...
    if (before)
    {
        ...
        /* Visibility changes to map objects affect every part and msp. */
        if (flagno == FLAG_IS_INVISIBLE &&
            op->map)
        {
            FOREACH_PART_OF_OBJECT(this, head, next)
            {
                msp_t *msp = MSP_KNOWN(this);

                hooks->msp_rebuild_slices_without(msp, this);
                SET_OR_CLEAR_FLAG(this, flagno, value);
                hooks->msp_rebuild_slices_with(msp, this);
                OBJECT_UPDATE_VIS(this);
            }
    ...
    else
    {
        ...
        FOREACH_PART_OF_OBJECT(part, op, next)
        {
            ...
            if (part->map)
            {
                ...
                MSP_UPDATE(mask, part);

                if (mask)
                {
                    MSP_KNOWN(part)->flags |= mask;
                }
            }
...
static int GameObject_PickUp(lua_State *L)
{
    ...
    pickedup = hooks->thing_pick_up(WHO, WHAT, (where) ? where->data.object : NULL, nrof);
...
static int GameObject_Give(lua_State *L)
{
    ...
    given = hooks->thing_pick_up(whom->data.object, WHAT, NULL, nrof);
...
//    }
    ...
    else
    {
        ...
        else
        {
            ...
            (void)hooks->thing_pick_up(WHO, loot, NULL, 1);
            WHAT->value -= total;
            lua_pushboolean(L, 1);
        }
    }
...
static int GameObject_FindSkill(lua_State *L)
{
    ...
    get_lua_args(L, "Oi", &self, &nr);

    if (WHO->type != PLAYER)
    {
        luaL_error(L, "object:FindSkill(): Can only be called on a player!");
        return 0;
    }
    ...
    if (nr < 0 ||
        nr >= NROFSKILLS)
    {
        luaL_error(L, "object:FindSkill(): Skill nr out of range (is: %d, must be: 0-%d)!",
            nr, NROFSKILLS);
        return 0;
    }

    push_object(L, &GameObject, CONTR(WHO)->skill_ptr[nr]);
    return 1;
}
...
static int GameObject_Move(lua_State *L)
{
    lua_object *self;
    sint32      direction;
    lua_object *originator = NULL;
    sint32      terrain = -1;
    sint32      n;

    get_lua_args(L, "Oi|Oi", &self, &direction, &originator, &terrain);

    if (terrain > 0)
    {
        uint16 t = WHO->terrain_flag;

        WHO->terrain_flag = (uint16)terrain;
        n = hooks->move_ob(WHO, direction, (originator) ? originator->data.object : NULL);
        WHO->terrain_flag = t;
    }
    else
    {
...
static int GameObject_IdentifyItem(lua_State *L)
{
    lua_object   *self;
    lua_object   *where;
    lua_object   *what = NULL;
    int           mode;
    sint32        nrof;
    objectlink_t *list = NULL;

    get_lua_args(L, "OO|Oi", &self, &where, &what, &mode);
    nrof = hooks->thing_declare(WHO, where->data.object,
        (what) ? what->data.object : NULL, WHO->level,
        THING_DECLARE_BIT_DETECT | THING_DECLARE_BIT_IDENTIFIED, &list);

    if (nrof < 0)
    {
        return luaL_error(L, "object:identifyItem() failed sanity test!");
    }
    else if (nrof > 0)
    {
        sint32 i = 0;
        ...
        while (list)
        {
            ...
            push_object(L, &GameObject, link->objlink.ob);
            lua_rawseti(L, -2, ++i);
            hooks->objectlink_delink(&list, NULL, link);
        }
...
/*****************************************************************************/
static int GameObject_MakeEgo(lua_State *L)
{
    lua_object *self;
    int         mode;
    char       *name = NULL;
    ...
    get_lua_args(L, "Oi|sb", &self, &mode, &name, &lock);

    if (WHO->type == PLAYER)
    {
        return luaL_error(L, "object:MakeEgo() must not be called on a player!");
    }

    if (QUERY_FLAG(WHO, FLAG_EGOLOCK) &&
        WHO->egomode >= 0)
    {
        lua_pushboolean(L, 0);
        return 1;
    ...
    if (mode <= 0 ||
        !name)
    {
        WHO->egomode = (mode != 0) ? EGOITEM_MODE_UNBOUND : 0;

        if (name &&
            strcmp(name, hooks->shstr_cons->any) == 0 && // name is not shstr yet
            WHO->type == MONSTER)
        {
            SHSTR_FREE_AND_ADD_STRING(WHO->egoname, name);
        }
        else
        {
            SHSTR_FREE(WHO->egoname);
        }
    }
    else if (strcmp(name, hooks->shstr_cons->any) == 0 && // name is not shstr yet
        (WHO->type != MONSTER ||
         mode == EGOITEM_MODE_PLAYER))
    {
        return luaL_error(L, "object:MakeEgo(): Arg #2 must not be \"*\" when object is not a monster or arg #1 is EGOITEM_MODE_PLAYER (1)!");
    }
    else
    {
        WHO->egomode = MIN(EGOITEM_MODE_CLAN, mode);
        SHSTR_FREE_AND_ADD_STRING(WHO->egoname, name);
    }

    if (lock != 0)
    {
        SET_FLAG(WHO, FLAG_EGOLOCK);
    }
...
static int GameObject_UpdateQuest(lua_State *L)
{
    ...
    {
        int b = hooks->update_quest(quest, SUBTYPE_QUEST_UPDATE_ARBITRARY, NULL,
                                    text, NULL);
