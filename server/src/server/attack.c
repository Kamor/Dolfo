...
#define HIT_FLAG_NO_DISPLAY    1024
/* HIT_FLAG_WEAPON      ==    2048 is defined in global.h */
...
int attack_ob(object_t *target, object_t *hitter, object_t *hit_obj)
{
    ...
    if (plugin_trigger_object_event(PLUGIN_EVENT_OBJECT_ATTACK, target, hitter, hit_obj, NULL, 0, 0, 0))
        goto error;
    ...
    if (hitter->type == PLAYER && hit_obj == hitter)
    {
...
int damage_ob(object_t *op, int dam, object_t *hitter, attack_envmode_t env_attack)
{
    ...
    */
    ...
    {
								// we don't use num and chance anymore, we also ignore hardcoded material_base flags below
								// we don't send num anymore, we send damage, lets try to send total damage first
        ...
        if(op->type == PLAYER)
        {
            ...
            material_attack_damage(op, dam, chance, flags); // here we send damage now
        }
        ...
        if(hitter->type == PLAYER)
        {
            ...
            material_attack_damage(hitter, dam, chance/2, flags); // here we send damage now
        }
    }
    ...
    if (op->damage_round_tag != ROUND_TAG && !(flags & HIT_FLAG_NO_DISPLAY))
    {
        op->last_damage = 0;
...
sint32 hit_map(object_t *hitter, msp_t *msp)
{
    ...
    FOREACH_OBJECT_IN_MSP(this, msp, next)
    {
        ...
        if (this == hitter ||                                 // don't hurt self
            ...
            (this->type != PLAYER &&
             this->type != MONSTER))                                   // can only hit live objects (for now)
        {
            continue;
        }

        if (!plugin_trigger_object_event(PLUGIN_EVENT_OBJECT_ATTACK, this, hitter, hitter, NULL, 0, 0, 0))
        {
...
static int HitPlayerAttacktype(object_t *op, object_t *hitter, int *flags, int damage, uint32 attacknum, int magic)
{
    ...
    if (attacknum == ATNR_INTERNAL)
    {
        ...
        if (hitter->type == FORCE &&
            hitter->subtype == SUBTYPE_FORCE_POISON) /* we have a poison force object (thats the poison we had inserted) */
        {
...
object_t * hit_with_arrow(object_t *op, object_t *victim)
{
    ...
    if (!plugin_trigger_object_event(PLUGIN_EVENT_OBJECT_ATTACK, victim, owner, op, NULL, 0, 0, 0))
    {
    ...
    if (hit_something)
    {
        plugin_trigger_object_event(PLUGIN_EVENT_OBJECT_STOP, op, victim, NULL, NULL, 0, 0, 0);
