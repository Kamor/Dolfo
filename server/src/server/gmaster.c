...
command_setserverexp(object_t *op, char *params)
{
    if (!op)
    ...
    int i;
    if (sscanf(params, "%d", &i) != 1)
        return COMMANDS_RTN_VAL_SYNTAX;

    int exp = MAX(-100, MIN(i, 1000)); // -100% is 0 exp gain

    if (exp!=i)
      ndi(NDI_UNIQUE, 0, op, "adjusted to range -100%% - 1000%%");

    // set globalserverexp
    global_exp_bonus = exp;

    if (global_exp_bonus>0) // we use green for +
          ndi(NDI_UNIQUE, 0, op, "Server : ~+%d%%~ exp bonus.", global_exp_bonus);
    else                    // and todo red color here?
          ndi(NDI_UNIQUE, 0, op, "Server : %d%% exp bonus.", global_exp_bonus);
    return COMMANDS_RTN_VAL_OK;
}

command_setplayerexp(object_t *op, char *params)
{
    player_t     *pl;
    char    player_name[20];
    ...
    if (sscanf(params, "%s %d", player_name, &i) != 2)
           return COMMANDS_RTN_VAL_SYNTAX;
    ...
    int exp = MAX(-100, MIN(i, 1000));
    // could be interesting to go to -100 for a kind of some minutes cursed exp before townraid

    if (exp!=i)
      ndi(NDI_UNIQUE, 0, op, "adjusted to range -100%% - 1000%%");

    pl->exp_bonus=exp;
    if (pl->exp_bonus>0) // we use green for +
          ndi(NDI_UNIQUE, 0, op, "Player : ~+%d%%~ exp bonus.", pl->exp_bonus);
