...
/* FIXME: This is already handled by the caller. */
#define MOVEAPPLYBYFLAGS(_O1_, _O2_, _F_) \
    ((!((_F_) & MOVE_FLAG_VANISHED) && \
      ((IS_AIRBORNE((_O2_)) && \
        ((((_F_) & MOVE_FLAG_FLY_ON) && \
          QUERY_FLAG((_O2_), FLAG_FLY_ON)) || \
         (((_F_) & MOVE_FLAG_FLY_OFF) && \
          QUERY_FLAG((_O2_), FLAG_FLY_OFF)))) || \
       (!IS_AIRBORNE((_O1_)) && \
        ((((_F_) & MOVE_FLAG_WALK_ON) && \
          QUERY_FLAG((_O2_), FLAG_WALK_ON)) || \
         (((_F_) & MOVE_FLAG_WALK_OFF) && \
          QUERY_FLAG((_O2_), FLAG_WALK_OFF)))))) \
     ? 1 : 0)
...
void move_apply(object_t *const trap_obj, object_t *const victim, object_t *const originator, const uint16 flags)
{
    ...
    /* If victim is entity and trap is bound to an ego shared by victim, do
     * nothing. */
    if ((victim->type == PLAYER ||
         victim->type == MONSTER) &&
        trap->egomode > 0 &&
        egoitem_check(victim, trap, 1) == 0)
    {
        return;
    }
    ...
    switch (trap->type)
    {
        ...
        case CHECK_INV:
        if (MOVEAPPLYBYFLAGS(victim, trap, flags) &&
            (victim->type == PLAYER ||
             victim->type == MONSTER))
        {
            (void)check_inv(victim, trap);
        }
        ...
        case DIRECTOR:
          if (victim->direction)
          {
              ...
              OBJECT_TURN(victim, trap->direction);
          }
          goto leave;
        ...
        case BULLET:
          if (!(flags & MOVE_FLAG_VANISHED) &&
              (QUERY_FLAG(victim, FLAG_NO_PASS) ||
               victim->type == PLAYER ||
               victim->type == MONSTER))
          {
...
int apply_object(object_t *who, object_t *what, int aflag)
{
    ...
    if (pl)
    {
        ...
        else if (!QUERY_FLAG(what, FLAG_APPLIED))
        {
            ...
            else if (egoitem_check(who, what, -1) != 0)
            {
                return 4;
            }
        }
...
/* TODO: Desperately needs a rewrite (make it player-only and use
 * pl->equipment[] shortcuts).
 *
 * -- Smacky 20200509 */
int apply_equipment(object_t *who, object_t *what, int aflags)
{
    ...
    if (QUERY_FLAG(what, FLAG_APPLIED))
    {
        ...
        switch (what->type)
        {
            ...
            case HORN:
                ...
                if(what->type != ARROW || what->subtype > 127)
                {
                    if (!pl)
                    {
    ...
    /* Check that who does not already have the same type as what applied and
     * if so, unapply first. */
    /* All these types use the same equipment slot. */
    /* TODO: ARROW > 127 is a thrown weapon. Would it not be simpler to  have
     * type AMMO and type THROWING_WEAPON?
     *
     * -- Smacky 20200511 */
    sameslot = (what->type == HORN ||
        what->type == ROD ||
        what->type == WAND ||
        what->type == BOW ||
        (what->type == ARROW &&
         what->subtype > 127)) ? 1 : 0;

    /* Allow multiple rings. Currently we allow 2 but perhaps more in future.
     * But this is a game balance issue and technically there'd be more to do
     * than just increasing the number here. */
    rings = 0;
    ...
    switch (what->type)
    {
        ...
        case WEAPON:
            ...
            if (pl &&
                ...
                (what->subtype >= WEAP_POLE_IMPACT ||
                 what->subtype >= WEAP_2H_IMPACT))
            {
                if (!(aflags & AP_QUIET))
                {
        ...
        case SHIELD:
            ...
            if (pl &&
                ...
                (pl->equipment[PLAYER_EQUIP_WEAPON1]->subtype >= WEAP_POLE_IMPACT ||
                 pl->equipment[PLAYER_EQUIP_WEAPON1]->subtype >= WEAP_2H_IMPACT))
            {
                if (!(aflags & AP_QUIET))
                {
    ...
    switch (what->type)
    {
        ...
        case ARROW:
            ...
            */
            if(what->subtype < 127) /* its amunition */
            {
                ...
                if (pl &&
                    (!pl->equipment[PLAYER_EQUIP_BOW] ||
                     ...
                     pl->equipment[PLAYER_EQUIP_BOW]->subtype != what->subtype))
                {
                    if (!(aflags & AP_QUIET))
                    {
...
void turn_off_light(object_t *op)
{
    ...
    if (op->other_arch && op->other_arch->clone.subtype & 1)
    {
