...
int container_link(player_t *const pl, object_t *const sack)
{
    ...
    */
    if ((sack->event_flags & PLUGIN_EVENT_FLAG(PLUGIN_EVENT_SPECIAL_QUEST)))
    {
        object_t *this,
                 *next;
        ...
        FOREACH_OBJECT_IN_OBJECT(this, sack, next)
        {
            if (this->type == TYPE_QUEST_TRIGGER &&
                (this->subtype == SUBTYPE_QUEST_TRIGGER_NORMAL ||
                 this->subtype == SUBTYPE_QUEST_TRIGGER_ITEM))
            {
                insert_quest_item(this, pl->ob);
            }
        }
    }
    ...
    else /* we are the first one opening this container */
    {
				...
				// todo transfer this to a kind of trap_auto_check function
				object_t *skill_find_traps = pl->skill_ptr[SK_FIND_TRAPS];
				object_t *skill_disarm_traps = pl->skill_ptr[SK_REMOVE_TRAP];

				object_t *that = sack->inv;
				/* check inventory of that. */
				int break_opening = 0;
				while (that)
				{
						//if (QUERY_FLAG(that, FLAG_IS_TRAPED))
						if (that->type == RUNE && that->stats.Cha <= 1)
						{
								if (skill_disarm_traps)
								{
										ndi(NDI_UNIQUE, 0, pl->ob, "You try to disarm a trap.");
										trap_disarm(pl->ob, that, skill_disarm_traps->level);
										// break_opening = 1; // we don't break here anymore, player has one try to disarm this trap
								}
						}
						else if (that->type == RUNE && that->stats.Cha > 1)
						{
								if (skill_find_traps)
								{
										if (trap_find(pl->ob, that, skill_find_traps->level))
										{
												trap_show(that, sack);
												break_opening = 1;
										}
					...
					if (break_opening)
					{
							ndi(NDI_UNIQUE, 0, pl->ob, "You stop opening the %s.",STRING_OBJ_NAME(sack));
							CLEAR_FLAG(sack, FLAG_BEEN_APPLIED); // when found a trap, we don't want corpse flag as "searched"
							pl->container=NULL; // we also don't want the player to linked to the container, outer calling function is checking
							return 1;
					}
