...
int command_help(object_t *op, char *params)
{
    player_t *pl;
    int       len = 0;
    char      readbuf[SMALL_BUF];
    char      scriptbuf[SMALL_BUF + 6]; // add +6 for "/help "
    ...
    if (!params)
    {
        trigger_object_plugin_event(EVENT_TALK, op, op, NULL, "/help",
                                    NULL, NULL, NULL, SCRIPT_FIX_ACTIVATOR, "/lua/help/help.lua");
    ...
    // Basic but thorough sanitization. Stop reading input if the user types anything but A-Za-z0-9 or space.
    while (isalnum(params[len]) || params[len] == ' ' || params[len] == '/')
    {
        readbuf[len] = params[len];
        len++;
    ...
    if (readbuf)
    {
        sprintf(scriptbuf, "/help %s", readbuf);
        trigger_object_plugin_event(EVENT_TALK, op, op, NULL, scriptbuf,
                                    NULL, NULL, NULL, SCRIPT_FIX_ACTIVATOR, "/lua/help/help.lua");
        return 0;
    }
    else
...
command_bonus(object_t *op, char *params)
{
    // we don't need params here, we can ignore
    if (!op)
        return COMMANDS_RTN_VAL_ERROR;

    if (global_exp_bonus>0) // we use green for +
          ndi(NDI_UNIQUE, 0, op, "Server : ~+%d%%~ exp bonus.", global_exp_bonus);
    else                    // and todo color for 0 and -
          ndi(NDI_UNIQUE, 0, op, "Server : %d%% exp bonus.", global_exp_bonus);

    if (CONTR(op)->exp_bonus>0) // we use green for +
          ndi(NDI_UNIQUE, 0, op, "Player : ~+%d%%~ exp bonus.", CONTR(op)->exp_bonus);
    else                    // and todo color for 0 and -
          ndi(NDI_UNIQUE, 0, op, "Player : %d%% exp bonus.", CONTR(op)->exp_bonus);

    // todo guild exp bonus
