...
char *examine_monster(object_t *op, object_t *tmp, char *buf, int flag)
{
    ...
    {
        if (walk->type == FORCE &&
            walk->subtype == SUBTYPE_FORCE_POISON)
        {
...
// simplified examine logic for items
char *examine_item(object_t *op, object_t *item)
{
    ...
    *buf='\0';
    ...
    sprintf(buf, "%s %s",
           (item->nrof > 1) ? "Those are" : "That is",
            query_name(item, op, item->nrof, 1));
    ...
    if (item->item_level)
    {
        if (item->item_skill)
        {
            sprintf(buf, "It needs a level of %d in %s to use.\n", item->item_level,
                STRING_SAFE(CONTR(op)->skillgroup_ptr[item->item_skill-1]->name));
            strcat(buf_out, buf);
        }
        else
        {
    ...
    sprintf(buf, "Qua: %d Con: %d%%.\n", item->item_quality, item->item_condition);
    strcat(buf_out, buf);

    // no immunities, for what, when we have no items using this?

    if (item->weight)
    {
        sprintf(buf, item->nrof > 1 ? "They weigh %3.3f kg.\n" : "It weighs %3.3f kg.\n",
            (float) (item->nrof > 1 ? item->weight * (sint32)item->nrof : item->weight) / 1000.0f);
        strcat(buf_out, buf);
    }
...
char *examine(object_t *op, object_t *tmp, int flag)
{
    ...
    *buf_out='\0';
    if(op)
    {
        if (plugin_trigger_object_event(PLUGIN_EVENT_OBJECT_EXAMINE, tmp, op, NULL, NULL, 0, 0, 0))
        {
            return NULL;
        }
    }
    ...
    if (tmp->map)
    {
        ...
        if (shop && QUERY_FLAG(shop, FLAG_IDENTIFIED) && QUERY_FLAG(tmp, FLAG_UNPAID))
        {
            THING_IDENTIFY(tmp);
        }
    }
    ...
    *buf='\0';
    ...
    if (!QUERY_FLAG(tmp, FLAG_UNIDENTIFIABLE) &&
    ...
    else if (QUERY_FLAG(tmp, FLAG_IDENTIFIED))
    {
        ...
        if (tmp->item_quality)
        {
            ...
            // we have not one item in game using this flags, but need this code?
    ...
    switch (tmp->type)
    {
        case ANVIL:
            if (tmp->stats.food == -1)
            {
                sprintf(buf, "It is a very sturdy anvil.\n");
            }
            else
            {
                sprintf(buf, "It looks like it can handle about %d more uses.\n", tmp->stats.food);
            }
            break;
        case SPELLBOOK:
          if (QUERY_FLAG(tmp, FLAG_IDENTIFIED) && tmp->stats.sp >= 0 && tmp->stats.sp <= NROFREALSPELLS)
          {
              if (tmp->subtype == SUBTYPE_SPELLBOOK_CLERIC)
