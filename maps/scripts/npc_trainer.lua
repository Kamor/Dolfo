...
local function npc_fee(costs)
  ...
  costs = costs + costs / 100 * npc.level
  return costs
end

local function to_pay(skill)
  local sobj = player:GetSkill(game.TYPE_SKILL, skill)
...
function topic_training() -- we need this global
  ...
  for index, data in to_train do
    ...
    if skill_number==-1 then
      ...
      local skill = player:GetSkill(game.TYPE_SKILL, skill_number)
      if skill == nil then
        ib:AddSelect("Learn " .. data.skill_name .. " - " .. player:ShowCost(npc_fee(500)) , "learn " .. index, data.icon, "I can teach you ".. data.skill_name)
      else
          ...
          if (slevel<=npc.level) then
            ...
            if skill_group ~= nil then
              ...
              if skill_group.level >= slevel then
                ...
                ib:AddSelect("Train " .. data.skill_name .. " Level " .. slevel .. " - " .. player:ShowCost(npc_fee(to_pay(skill_number))) , "services" , data.icon, "Train first more " .. skill_group.name)
              end
            else
...
local function topic_learn(index)
  ...
  if index~=nil then
    ...
    if i>0 and i <= getTableSize(to_train) then
      ...
      if skill_number~=-1 then
        if player:GetSkill(game.TYPE_SKILL, skill_number) ~= nil then
          ib:SetMsg("\nYou like to troll me?.\nYou already know this skill.")
          return
        end

        if player:PayAmount(npc_fee(500)) == 1 then -- 5 silver + npc fee
          player:AcquireSkill(skill_number, game.LEARN)
...
local function topic_train(index)
  ...
  if index~=nil then
    ...
    if i>0 and i <= getTableSize(to_train) then
      local skill_number=game:GetSkillNr(to_train[i].skill_name)
      if skill_number~=-1 then
        skill = player:GetSkill(game.TYPE_SKILL, skill_number)
        if skill == nil then
        ...
        if skill_group ~= nil then
          ...
          if skill_group.level >= slevel then
            if player:PayAmount(npc_fee(to_pay(skill_number))) == 1 then
              ...
              player:SetSkill(game.TYPE_SKILL, skill_number, slevel, 0)
